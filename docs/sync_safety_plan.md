...
已完成的模拟器证据：最新 APK 使用 `adb install -r` 保留现有 APP 数据覆盖安装后可正常启动，日志没有 Android 或 Flutter 崩溃。模拟器中没有证据证明原数据是一套真实的非空旧版同步库，因此该项仍不能代替实体升级验收。

## 后续顺序

Windows 已在启动时检测 `ffplay` 和 `ffprobe`，并把真实能力显示在底部播放器。P0 只剩真机断网、实体覆盖安装保留播放和主端删除回归。P1 才考虑同步进度、取消、失败项重试，以及是否用应用内播放器后端取代外部 `ffplay`；P2 才考虑排序、后台同步、性能与无障碍。V1 不扩展云端或双向同步。
