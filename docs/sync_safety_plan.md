...
已完成的模拟器证据：最新 APK 使用 `adb install -r` 保留现有 APP 数据覆盖安装后可正常启动，日志没有 Android 或 Flutter 崩溃。正式 Windows EXE 与 `emulator-5554` 已完成真实跨进程连接、整张歌单同步和重复同步；Android 最终保持 1 首歌曲、1 张歌单和 1 个正式音频文件，`.sync_staging` 为空。关闭同步端口并完全退出 Windows EXE 后，Android 仍保有音频焦点，暂停按钮保持可用且播放进度持续前进。模拟器仍不能替代实体手机相机、二维码扫描、真实 Wi-Fi 和实体覆盖安装验收。

## 后续顺序

Windows 已在启动时检测 `ffplay` 和 `ffprobe`，并把真实能力显示在底部播放器。P0 只剩实体手机扫码/断网、实体覆盖安装保留播放和主端删除回归。P1 才考虑同步进度、取消、失败项重试，以及是否用应用内播放器后端取代外部 `ffplay`；P2 才考虑排序、后台同步、性能与无障碍。V1 不扩展云端或双向同步。
