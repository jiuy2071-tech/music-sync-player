...
Windows 已在启动时检测 `ffplay` 和 `ffprobe`，并把真实能力显示在底部播放器。P0 只剩真机断网、旧数据库升级和主端删除回归。P1 才考虑同步进度、取消、失败项重试，以及是否用应用内播放器后端取代外部 `ffplay`；P2 才考虑排序、后台同步、性能与无障碍。V1 不扩展云端或双向同步。
