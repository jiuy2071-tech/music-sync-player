...
自动代码安全项、旧数据库结构升级测试、模拟器跨进程同步、空歌单、主端删除对账与离线播放已经闭环。下一步只需在实体手机确认相机扫码、真实 Wi-Fi、覆盖安装和断网离线播放体感。详细设计与负面测试见 `docs\sync_safety_plan.md`。保持 V1 范围，不扩展云同步、远程播放或 Android 主库编辑。
